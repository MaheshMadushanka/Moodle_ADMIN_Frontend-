...
function FiltersBar({ filters, onFilterChange, onExportCSV, onExportPDF, isDarkMode }) {
  return (
    <div className={`border rounded-lg p-4 sm:p-6 mb-8 transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              className={`px-3 py-2.5 rounded-lg border transition-colors text-sm min-h-[44px] ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300 text-slate-900'} focus:outline-none focus:ring-2 focus:ring-blue-500/20`}
            />
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <button
            onClick={onExportCSV}
            className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-colors text-sm min-h-[44px] ${isDarkMode ? 'bg-slate-700 hover:bg-slate-600 text-slate-200' : 'bg-slate-100 hover:bg-slate-200 text-slate-900'}`}
          >
            <Download size={16} />
            CSV
          </button>
