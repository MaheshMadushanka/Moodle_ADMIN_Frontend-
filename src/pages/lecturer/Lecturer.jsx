                ...
                {filtered.length > 0 ? (
                  filtered.map((l) => (
                    <tr key={l.id} className={`transition-colors duration-150 ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-slate-50'}`}>
                      <td className={`px-4 py-3 text-xs font-medium ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{l.regNumber}</td>
                      <td className={`px-4 py-3 text-sm ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{l.fullName}</td>
                      <td className={`px-4 py-3 text-xs ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{l.email}</td>
                      <td className={`px-4 py-3 text-xs ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{l.contact}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${l.mode === 'Online' ? (isDarkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-50 text-blue-700') : (isDarkMode ? 'bg-green-900/30 text-green-300' : 'bg-green-50 text-green-700')}`}>{l.mode}</span>
                      </td>
                      <td className={`px-4 py-3 text-xs ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{l.subject}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => setSelected(l)} className={`p-1.5 rounded transition-colors duration-200 min-h-[40px] min-w-[40px] flex items-center justify-center ${isDarkMode ? 'hover:bg-blue-900/30 text-blue-400' : 'hover:bg-blue-50 text-blue-600'}`} title="View">
                            <Eye size={16} />
                          </button>
                          <button onClick={() => handleDelete(l.id)} className={`p-1.5 rounded transition-colors duration-200 min-h-[40px] min-w-[40px] flex items-center justify-center ${isDarkMode ? 'hover:bg-red-900/30 text-red-400' : 'hover:bg-red-50 text-red-600'}`} title="Delete">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={7} className={`px-4 py-12 text-center text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>No lecturers found</td>
                  </tr>
                )}
              </tbody>
            </table>
              ...
              filtered.map((l) => (
                <div key={l.id} className={`p-4 border rounded-lg ${isDarkMode ? 'border-slate-700 bg-slate-700/50' : 'border-slate-200 bg-slate-50'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <p className={`font-bold text-sm ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{l.fullName}</p>
                      <p className={`text-xs ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{l.regNumber}</p>
                    </div>
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${l.mode === 'Online' ? (isDarkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-50 text-blue-700') : (isDarkMode ? 'bg-green-900/30 text-green-300' : 'bg-green-50 text-green-700')}`}>
                      {l.mode}
                    </span>
                  </div>
                  <div className="space-y-2 text-xs mb-4">
                    <p><strong>Email:</strong> <span className={isDarkMode ? 'text-slate-300' : 'text-slate-600'}>{l.email}</span></p>
                    <p><strong>Contact:</strong> <span className={isDarkMode ? 'text-slate-300' : 'text-slate-600'}>{l.contact}</span></p>
                    <p><strong>Subject:</strong> <span className={isDarkMode ? 'text-slate-300' : 'text-slate-600'}>{l.subject}</span></p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => setSelected(l)} className={`flex-1 py-2.5 rounded text-sm font-medium transition-colors min-h-[44px] ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
                      View
                    </button>
                    <button onClick={() => handleDelete(l.id)} className={`flex-1 py-2.5 rounded text-sm font-medium transition-colors min-h-[44px] ${isDarkMode ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-red-600 hover:bg-red-700 text-white'}`}>
                      Delete
                    </button>
                  </div>
                </div>
            ...
            <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-lg w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto p-6`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="mt-6 flex flex-col-reverse sm:flex-row gap-3 justify-end">
                  <button type="button" onClick={() => setShowAdd(false)} className={`px-6 py-2.5 rounded-lg font-medium transition-colors min-h-[44px] ${isDarkMode ? 'bg-slate-700 hover:bg-slate-600 text-white' : 'bg-slate-200 hover:bg-slate-300'}`}>
                    Cancel
                  </button>
                  <button type="submit" className="px-6 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors min-h-[44px]">
                    Save
                  </button>
                </div>
              </form>
            ...
            <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-lg w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto p-6`}>
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                <div>
                  ...
                  <p className={`text-sm font-semibold mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{selected.fullName}</p>
                </div>
                <div>
                  <p className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Reg. Number</p>
                  <p className={`text-sm font-semibold mt-1 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{selected.regNumber}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className={`text-sm mt-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{selected.contact}</p>
                </div>
                <div>
                  <p className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Mode</p>
                  <p className={`text-sm mt-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{selected.mode}</p>
                </div>
                <div>
